#!/bin/bash
#=============================================================================
#     FileName: screenfetch.sh
#         Desc: This is a copy from https://github.com/theshadowfog/dotfiles/blob/master/bin/screenfetch
#=============================================================================
#
# screenFetch
#
...
display=( host distro kernel uptime pkgs shell res wm cpu mem )
...
# WM & DE process names
# Removed WM's: compiz
wmnames="fluxbox openbox blackbox xfwm4 metacity kwin icewm pekwm fvwm dwm awesome wmaker stumpwm xmonad musca i3 ratpoison scrotwm spectrwm wmfs wmii beryl subtle e16 enlightenment sawfish emerald monsterwm dminiwm"
...
detectdistro () {
	if [[ -z $distro ]]; then
		...
		if which lsb_release >/dev/null 2>&1; then
			...
			if [ "$distro_detect" == "Viperr" ]; then
				distro="Viperr"
				distro_codename=null
			fi
			if [ "$distro_detect" == "SUSE LINUX" ]; then distro="openSUSE"; fi
			if [ "$distro_detect" == "ParabolaGNU/Linux-libre" ]; then distro="ParabolaGNU/Linux-libre"; fi
			if [ "$distro_detect" == "LinuxDeepin" ]; then
				distro="LinuxDeepin"
				distro_codename=null
			fi
			if [ "$distro_detect" == "Chakra" ]; then
				distro="Chakra"
				distro_release=null
			fi
		...
		if [ "$distro" == "Unknown" ]; then
			...
			if grep -q "LinuxDeepin" /etc/issue; then distro="LinuxDeepin"; fi
